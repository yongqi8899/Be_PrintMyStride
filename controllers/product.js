...
export const getAllProducts = asyncHandler(async (req, res, next) => {
  const products = await Product.find().populate({ path: 'user', strictPopulate: false });
  res.json(products);
});

export const createProduct = asyncHandler(async (req, res, next) => {
  const { body, userId } = req;
  const newProduct = await (await Product.create({ ...body, user: userId })).populate({ path: 'user', strictPopulate: false });
...
export const getSingleProduct = asyncHandler(async (req, res, next) => {
  ...
  const product = await Product.findById(id).populate({ path: 'user', strictPopulate: false });
...
export const deleteProduct = asyncHandler(async (req, res, next) => {
  ...
  if (!isValidObjectId(id)) throw new ErrorResponse('Invalid id', 400);
  const deletedProduct = await Product.findByIdAndDelete(id).populate({ path: 'user', strictPopulate: false });
